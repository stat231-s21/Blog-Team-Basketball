...
## Data Sources
The dataset we utilized for this investigation was scraped from basketball-reference.com. We scraped each team's roster from 2006-2021, which high school and college each player attended, and some relevant career statistics for each player. In total, 8657 instances of a player were scraped. 
...
* Damian Lillard and Steph Curry are great examples of players that are successful on a single team throughout their career (thusfar). 
...
* This netowrks shows a different set of players than the previous two, this network shows players with just very good field goal percentage. Therefore, they are not the 'superstars' that we were previously looking at and you can see how the connection type has changed. No longer are we looking at a few teams that are passing star players between them, but now each player playing for a few teams throughout their career. 
...
# get_diameter(cs0)
```
* The diameter of this network is `r get_diameter(cs0)`. 
*
