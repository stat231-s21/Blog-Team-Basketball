...
# Background
## Background on Topic
Basketball players play in one of 30 NBA teams. Players can leave a team via a trade (where a team exchanges the player for another), being waived and re-signed by another team (typically when a player under preforms), being drafted into a team, or being signed as a free agent at the end of a contract (or at the beginning of a career). 

As for colleges, typically only the "star" players tend to make it to the NBA. There are, however, some exceptions as some programs are notorious for creating NBA talent. Players can transfer between schools and have the ability to choose where to attend. 

## Data Sources
The dataset we utilized for this investigation was scraped from basketball-reference.com. We scraped each team's roster from 2006-2021, which college each player attended, and some relevant career statistics for each player. In total, 8657 instances of a player were scraped. 

# Findings
## Success
We begin our search for these answers by attempting to quantify player success through various (mostly offensive) game statistics. As mentioned above, the standard that we, as your authors and data scientists, chose to measure success by is VORP due to the fact it is a calculated statistic that attempts to weigh many simple stats. This algorithm, which is included in full in the Reference section, attempts to coerce basketball success into one simple number by weighing the perceived importance of various statistics (such as points per game (PPG) and total rebounds per game (TRB)) in addition to the pure statistics of each player's career. BPM, or Box Plus/Minus, is another statistic we decided to include as it is another option for a calculated box score based metric. It is another weighted estimate of a player's contribution to their team. Where BPM just looks at how much a player helps or hurts their team when they are on the court, VORP takes that and weighs by the number of minutes the player is on the court for, with the idea that better players are playing more minutes. We urge you to take a strong look at the VORP and BPM tabs especially if you are less familiar with basketball. You are also welcome to look into the purer career statistics we have included, notably field goal percentage, PPG, and TRB. 
...
## Networks
Network science is a field that studies links, called edges, between distinct elements, called nodes or vertices. By analyzing these links, one can learn information about the relationship between nodes. 

Specifically in these networks we are looking into the question - do more connected players have more successful NBA careers? Below we have attempted to build networks that allow you to visualize this phenomena. 
...
```
* The diameter of this network is `r get_diameter(V)`. 
* There are a fair number of connections here, as there was a lot of movement with Kevin Durant, Russell Westbrook, and James Harden. These three players show that 
...
* However, the argument could be made that this network shows that great players tend to gravitate towards each other, support for a common phenomena happening in the NBA currently. 
...
```
* The diameter of this network is `r get_diameter(B)`. 
* Another very connected network, most players have played for more than one team, and those are often the same teams. 
...
```
* The diameter of this network is `r get_diameter(FG)`. 
* This networks shows a different set of players than the previous two, this network shows players with just very good field goal percentage. Therefore, they are not the 'superstars' that we were previously looking at and you can see how the connection type has changed. No longer are we looking at a few teams that are passing star players between them, but now each player playing for a few teams throughout their career. 
...
```
* The diameter of this network is `r get_diameter(P)`. 
...
* **This introduces us to another point in this exploration - moving teams, or connection, is often determined by the team not the player.** 
...
```
* The diameter of this network is `r get_diameter(R)`. 
* Our rebound network is another that features a large diameter, with the possible reasoning that teams care less about retaining players that secure a high number of rebounds than a high points. 
...
```
* The diameter of this network is `r get_diameter(A)`. 
* This is our biggest network by far at a diameter of 14. We could easily draw the same conclusion as above with some knowledge on salary data for these players. 
...
# get_diameter(cs0)
```
We have chosen to isolate these players as they all have similar characteristics. As all shooting guards or point guards with similar statistics and careers of the same length, they are an easy group to look at in both the Shiny application and compare in our networks. All of these players have VORPs between 16 and 40, indicating that they are consistently good players but by no means stars of their teams. Their raw back this number up, they make on average 50% of the shots they attempt, average between 10 and 20 points a game, and between 2 and 8 assists and rebounds per game as well. 
Most importantly, they have each been on at least two different teams, and in the case of Rondo 7 different teams. This is a fair indication that they are not seen as 'essential' players to their teams, unlike star players like Lillard who is Portland's darling. These players have been on an average of 3 teams in their 15 year careers, approximately a new team every 5 years. This is a good indication of the way the NBA views its mid-tier players, as assets but movable ones. 
